 ...
 * @return {Function}
 */
exports.init = function (app, express, opt) {

  // generate resources
  generate(opt);

  // Serve up swagger ui static assets
  var swHandler = express.static(path.join(__dirname, '../../node_modules/swagger-ui/dist'));

  // Serve up swagger ui interface.
  var swaggerURL = new RegExp('^'+ opt.swaggerURL +'(\/.*)?$');

  app.get(swaggerURL, function (req, res, next) {
      if (req.url === opt.swaggerURL && !/url=/.test(req.url)) {
          res.writeHead(302, { 'Location' : req.url + '/?url=' + opt.swaggerJSON });
          res.end();
          return;
      }
